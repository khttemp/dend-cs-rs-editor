...
from importPy.tab1.notchWidget import *
from importPy.tab1.perfWidget import *
from importPy.tab1.hurikoWidget import *
from importPy.tab1.tab1EditWidget import *

from importPy.tab2.countWidget import *
from importPy.tab2.modelWidget import *
from importPy.tab2.fixedListWidget import *
from importPy.tab2.fixedList2Widget import *

from importPy.tab3.lensListWidget import *
from importPy.tab3.tailListWidget import *
...
def tab1AllWidget(tabFrame, decryptFile, trainIdx, varList, btnList, defaultData, widgetList, reloadFunc):    
    ...
    btnFrame.pack(anchor=NW, padx=10, pady=5)
    ...
    set_default_train_info_button.pack(side=LEFT, padx=15, pady=5)
    ...
    extract_csv_train_info_button.pack(side=LEFT, padx=15, pady=5)
    ...
    save_csv_train_info_button.pack(side=LEFT, padx=15, pady=5)

    v_edit = widgetList[0]
    v_edit.set("この車両を修正する")
    edit_button = ttk.Button(btnFrame, textvariable=v_edit, width=25)
    edit_button.pack(side=LEFT, padx=15, pady=5)
    ...
    edit_all_button.pack(side=LEFT, padx=15, pady=5)
    ...
    notchPerfFrame.pack(anchor=NW, padx=10, pady=5, expand=True, fill=BOTH)
...
def tab2AllWidget(tabFrame, decryptFile, trainIdx, game, widgetList, reloadFunc):
    ...
    tab_two_frame.pack(anchor=NW, fill=X)
    
    countModelLf = ttk.LabelFrame(tab_two_frame, text="車両", height=250)
    countModelLf.pack(anchor=NW, padx=10, pady=5, fill=X)
    ...
    edit_hensei_button.grid(columnspan=3, row=3, column=0, sticky=W+E, pady=15)

    edit_model_button = ttk.Button(countWidget.countFrame, text="モデル情報を修正")
    edit_model_button.grid(columnspan=3, row=4, column=0, sticky=W+E, pady=5)
    ...
    elseFrame.pack(anchor=NW, fill=X)
    ...
    elseFrame2.pack(anchor=NW, fill=X)
...
def tab3AllWidget(tabFrame, decryptFile, trainIdx, game, widgetList, reloadFunc):
    ...
    tab3frame.pack(anchor=NW, fill=BOTH, expand=True)
